...
function Checkout() {
  const { state, setDeliveryAddress, deliveryAddress, dispatch } = useData(); // Access dispatch from context
  ...
  const placeOrderHandler = () => {
    ...
    // Clear the cart from state and local storage
    dispatch({ type: "CLEAR_CART" }); // Clear cart in the global state
  ...
  return (
    <div className="cart-container flex-row-wrap middle-content">
      ...
      <div className="flex-column-start">
        ...
        <button
          className={`btn btn-primary order-button ${disable && "disabled"}`}
          disabled={disable}
